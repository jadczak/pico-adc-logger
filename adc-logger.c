...
int64_t debug_time_diff([[maybe_unused]] absolute_time_t start, [[maybe_unused]] absolute_time_t end)
{
#ifdef TIMING
...
void debug_uart_print([[maybe_unused]] uart_inst_t* uart, [[maybe_unused]] const char* str)
{
#ifdef TIMING
    uart_puts(uart, str);
#endif // TIMEING
...
bool sample_adc_callback(struct repeating_timer* t)
{
    ...
    data->min = min;
    data->sample = result;
    float voltage = (float)3.3 * (float)result / (1 << 12); // FIXME: ldrh.w r3, [r4, #64] @ 0x40 ???
    if (sprintf(debug_buf, "Average: Hex %04X Counts %4u Voltage %6.4F\r\n", result, result, voltage) < 0)
    { // FIXME: vldr s13 [pc, #180] 0x1000031c ???
        debug_uart_print(data->uart, error_string);
    }
    else
    {
...
#define UART_1382400 1382400
    ...
    struct adc_data data = {.buf = {}, .sample = 0, .err = 0, .uart = uart, .min = 0, .max = 0};
